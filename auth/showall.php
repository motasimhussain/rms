...
While($data = mysql_fetch_array($results,MYSQL_BOTH))  
	...
	echo "<tr>";
   	...
   	echo '<th width="10%">'.ereg_replace("_"," ",$parts[2]).'</th>';
	echo '<th width="5%">'.$parts[3].'</th>';
	echo '<th width="15%">'.ereg_replace("s","-",$parts[4]).strtoupper($parts[5]).'</th>';
	echo '<th width="5%">'.ereg_replace("_"," ",$parts[6]).'</th>';
	echo '<th width="20%">'.ereg_replace("_"," ",$parts[7]).'</th>';
	if($user_type=="admin"){
