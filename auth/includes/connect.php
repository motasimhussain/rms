...
{
    if ($type == "student") {
        $sql = mysql_query("SELECT * FROM student WHERE `seatno` = '$user' AND `password` = '$password'") or die (mysql_error());
    ...
    if ($type == "admin") {
        $sql = mysql_query("SELECT * FROM administrator WHERE `adminid` = '$user' AND `password` = '$password'") or die (mysql_error());
        $sql_results = mysql_fetch_array($sql);
        $user_exists = mysql_num_rows($sql);
        if ($user_exists == 1) {
